...
def train_model(args, model, data_loader, loss_criterion, optimizer):
    model.train()
    train_meter = Meter()
    total_loss = 0
    n_mol = 0
    for batch_id, batch_data in enumerate(data_loader):
        ...
        preds, weight,_ = model(rgcn_bg, rgcn_node_feats, rgcn_edge_feats, smask_feats)
...
def eval_model(args, model, data_loader, loss_criterion, out_path, seed=0):
    model.eval()
    smiles_list = []
    eval_meter = Meter()
    g_list = []
    ...
    if out_path is not None:
        object_array = np.array(smask_idx_list, dtype=object)

        np.save(out_path + '_mask_index.npy', object_array)
...
class BaseGNN(nn.Module):
    ...
    def __init__(self, gnn_rgcn_out_feats, ffn_hidden_feats, ffn_dropout=0.25, num_output=1, classification=True):
        ...
        self.predict = self.output_layer(ffn_hidden_feats, num_output)
    def forward(self, rgcn_bg, rgcn_node_feats, rgcn_edge_feats, smask_feats):
        """Multi-task prediction for a batch of molecules
        """
        # etype = rgcn_edge_feats.transpose(1, 0).sum(dim=1).mean(dim=1)          # 194
        # node_feats = rgcn_node_feats.transpose(1, 0).mean(dim=1)                # 149 * 40
        # Update atom features with GNNs
        # for rgcn_gnn in self.rgcn_gnn_layers:
        #     node_feats = rgcn_gnn(rgcn_bg, node_feats, etype)
        for rgcn_gnn in self.rgcn_gnn_layers:
        ...
        # graph_feats, weight = self.readout(rgcn_bg, node_feats, smask_feats)
...
class RGCN(BaseGNN):
    ...
    def __init__(self, ffn_hidden_feats, rgcn_node_feats, rgcn_hidden_feats, rgcn_drop_out=0.25, ffn_dropout=0.25,num_output=1,
                 classification=True):
