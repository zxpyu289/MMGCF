...
from data_loader import CondDataset, TrainCondDataset
...
def read_molecules(path):
    ...
    # exit_point = np.load(path + 'exit_point.npy', allow_pickle=True)
    full_smi = np.load(path + 'full_smi.npy')
    # length = np.load(path + 'gen_len.npy')
    # clique = np.load(path + 'clique.npy',  allow_pickle=True)

    f = open(path + 'config.txt', 'r')
    data_config = eval(f.read())
    f.close()

    return full_nodes, full_edges, full_smi, data_config

    # return full_nodes, full_edges, frag_nodes,frag_edges, length,exit_point, full_smi, clique, data_config
...
class Trainer(object):
    def __init__(self, dataloader, data_config, args, all_train_smiles=None):
        ...
        # if self.args.gpu:
        #     self._model = self._model.to(self.args.gpu)
    ...
    def train_epoch(self, epoch_cnt):
        ...
        for i_batch, batch_data in enumerate(self.dataloader):
            ...
            if args.warm_up:
                self._optimizer.param_groups[0]['lr'] = 0.002 * min((epoch_cnt + 1) ** (-0.5),
                                                                    (epoch_cnt + 1) * (5 ** (-1.5)))
            loss.backward()
        ...
        # print(epoch_example)
        print('Epoch: {: d}, loss {:5.5f}, epoch time {:.5f}, lr {:.4f}'.format(epoch_cnt, epoch_loss, time() - t_start,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compare_loss', action='store_true', default=True,
                        help='Save model only if current loss is lower than ever.')
    ...
    # full_nodes, full_edges, frag_nodes, frag_edges, gen_len, exit_point, full_smi, clique, data_config = read_molecules(args.path)
    # train_dataloader = DataLoader(TrainCondDataset(full_nfull_nodes, full_edges, full_smi, data_config = read_molecules(args.path)
