...
# from data_utils import save_one_mol
    ...
    def __init__(self, max_size, node_dim, bond_dim, edge_unroll, args):
        ...
        if self.node_masks is None:
            self.node_masks, self.adj_masks, \
                self.link_prediction_index, self.flow_core_edge_masks = self.initialize_masks(
    ...
    def forward(self, inp_node_features, inp_adj_features):
        ...
        if self.args.deq_type == 'random':
            ...
            inp_node_features_cont += self.args.deq_coeff * torch.rand(
                inp_node_features_cont.size()).cuda()  # (B, N, 14)
            inp_adj_features_cont += self.args.deq_coeff * torch.rand(
                inp_adj_features_cont.size()).cuda()  # (B, edge_num, 4)
            # inp_node_features_cont += self.args.deq_coeff * torch.rand(inp_node_features_cont.size()).to(self.args.gpu) # (B, N, 14)
            # inp_adj_features_cont += self.args.deq_coeff * torch.rand(inp_adj_features_cont.size()).to(self.args.gpu)  # (B, edge_num, 4)
    ...
    def initialize_masks(self, max_node_unroll=89, max_edge_unroll=12):
        ...
        num_masks = int(
            max_node_unroll + (max_edge_unroll - 1) * max_edge_unroll / 2 + (max_node_unroll - max_edge_unroll) * (
                max_edge_unroll))  # 1079
        num_mask_edge = int(num_masks - max_node_unroll)  # 404  #990

        node_masks1 = torch.zeros([max_node_unroll, max_node_unroll]).byte()  # ([89,89])
        adj_masks1 = torch.zeros([max_node_unroll, max_node_unroll, max_node_unroll]).byte()  # ([89,89, 89])
        node_masks2 = torch.zeros([num_mask_edge, max_node_unroll]).byte()  # ([990,89])
        adj_masks2 = torch.zeros([num_mask_edge, max_node_unroll, max_node_unroll]).byte()  # ([990, 89, 89])
        # is_node_update_masks = torch.zeros([num_masks]).byte()

        link_prediction_index = torch.zeros([num_mask_edge, 2]).long()  # ([990,2])

        flow_core_edge_masks = torch.zeros([max_node_unroll, max_node_unroll]).byte()  # ([89,89])
